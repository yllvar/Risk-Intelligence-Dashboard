export function MetricCard({ title, value, delta, stale = false }) {
  return (
...
export function RScoreCard({ score, stale = false }) {
  const getScoreColor = (s) => {
    if (s >= 0.7) return 'bg-red-100 text-red-800';
    if (s >= 0.4) return 'bg-amber-100 text-amber-800';
    return 'bg-green-100 text-green-800';
  };

  return (
    <div className={`p-6 rounded-xl shadow-sm transition-all ${stale ? 'bg-gray-100' : 'bg-white hover:shadow-md'}`}>
      <h3 className="text-sm font-medium text-gray-500 mb-1">Composite R-Score</h3>
      <div className="flex items-end justify-between">
        <p className={`text-2xl font-semibold ${stale ? 'text-gray-600' : 'text-gray-900'}`}>
          {score !== undefined ? score.toFixed(2) : '--'}
        </p>
        {score !== undefined && (
          <span className={`text-sm px-2 py-1 rounded-full ${getScoreColor(score)}`}>
